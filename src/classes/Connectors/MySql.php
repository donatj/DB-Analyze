...
abstract class MySql {
	...
	public static function make_connection() {
		...
		if( !self::$link ) trigger_error('Error Connecting to MySql ' . static::$_host, E_USER_ERROR);
		if( !mysql_select_db(static::$_database) ) trigger_error('Cannot Locate MySql: ' . static::$_database, E_USER_ERROR);
	...
	private static function error( $qry, $error, $fatal, $display = true ) {
		...
		if( $fatal ) {
			...
			if( is_object($_ms) && $display ) {
				$_ms->add($error . ' : ' . $qry . $trace_str, true);
			}
	...
	static function enumMembers( $table, $field ) {
		$row = self::fetch("show columns from " . $table . " where field = '" . self::input($field) . "'", conn::ROW);
